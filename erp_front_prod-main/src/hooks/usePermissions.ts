...
export const usePermissions = () => {
  ...
  const canAccessPage = useCallback((path: string): boolean => {
    ...
    // صفحة مراجعة إقرارات اختبار وزارة العمل
    if (path.startsWith('/dashboard/requests/ministry-exam-declarations-review')) {
      return userPermissions.hasPermission('dashboard.ministry-exam-declarations-review', 'view');
    }

    // صفحة رسوم اختبار وزارة العمل
    if (path.startsWith('/dashboard/requests/ministry-exam-fees')) {
      return userPermissions.hasPermission('dashboard.ministry-exam-fees', 'view');
    }

    // صفحة إقرار دخول اختبار وزارة العمل
    if (path.startsWith('/dashboard/requests/ministry-exam-declaration')) {
      return userPermissions.hasPermission('dashboard.ministry-exam-declarations', 'view');
    }
