...
export default function TraineePlatformReportPage() {
  ...
  const fetchAllPages = async (endpoint: string) => {
    ...
    while (hasNext) {
      ...
      const response = await fetchAPI(`${endpoint}${separator}page=${page}&limit=${limit}`);
      ...
      const explicitHasNext = typeof meta.hasNext === 'boolean' ? meta.hasNext : undefined;
      hasNext = explicitHasNext !== undefined ? explicitHasNext : page < totalPages;
  ...
  const loadData = async () => {
    try {
      ...
      const [traineesData, accountsData, settingsData] = await Promise.all([
        fetchAllPages('/trainees'),
        fetchAllPages('/trainee-platform/accounts'),
        fetchAPI('/settings')
      ]);
  ...
  const accountByTraineeId = new Map<number, TraineeAccount>();
  for (const account of accounts) {
    if (!accountByTraineeId.has(account.traineeId)) {
      accountByTraineeId.set(account.traineeId, account);
    }
  }

  const registeredTrainees = allTrainees.filter((trainee) => accountByTraineeId.has(trainee.id));

  const unregisteredTrainees = allTrainees.filter((trainee) => !accountByTraineeId.has(trainee.id));
  ...
  const getAccountInfo = (traineeId: number) => {
    return accountByTraineeId.get(traineeId);
  };
