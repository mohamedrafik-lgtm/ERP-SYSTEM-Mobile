...
import { ArrowRightIcon, PaperAirplaneIcon, AcademicCapIcon, DocumentTextIcon } from '@heroicons/react/24/outline';
import Link from 'next/link';
import { API_BASE_URL } from '@/lib/api';

export default function EnrollmentProofPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('تم إرسال طلب إثبات القيد بنجاح');
    ...
    <div className="max-w-3xl mx-auto pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8 mb-8">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-purple-50 rounded-2xl border border-purple-100 flex items-center justify-center flex-shrink-0">
            <AcademicCapIcon className="w-7 h-7 text-purple-600" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900">طلب إثبات قيد</h1>
            <p className="text-sm text-slate-500 mt-1">قم بتعبئة النموذج أدناه لطلب وثيقة رسمية تثبت قيدك في المعهد.</p>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            <textarea
              ...
              rows={5}
              placeholder="مثال: للتقديم في وظيفة، للبنك، أو أي جهة أخرى..."
              required
            />
            <DocumentTextIcon className="absolute top-3.5 left-4 w-5 h-5 text-slate-400" />
          </div>
          <p className="text-xs text-slate-500 mt-2">
            يرجى كتابة اسم الجهة التي سيتم توجيه إثبات القيد إليها لضمان دقة الوثيقة.
          </p>
        </div>
