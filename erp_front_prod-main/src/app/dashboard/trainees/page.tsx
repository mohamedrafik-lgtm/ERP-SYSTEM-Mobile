...
const isAllowedMinistryDeclarationFile = (file: File) => {
  if (file.type.startsWith('image/')) return true;
  return /\.(jpg|jpeg|png|gif|webp|bmp|heic|heif)$/i.test(file.name);
};
...
function TraineesPageContent() {
  ...
  const canViewTraineeArchive = userPermissions?.hasPermission('dashboard.trainees.archive', 'view') || false;
  ...
  // حالات تسليم إقرار وزارة العمل من الإدارة
  const [showMinistryDeliveryModal, setShowMinistryDeliveryModal] = useState(false);
  const [selectedTraineeForMinistryDelivery, setSelectedTraineeForMinistryDelivery] = useState<Trainee | null>(null);
  const [ministryDeliveryNotes, setMinistryDeliveryNotes] = useState('');
  const [ministryScannedFile, setMinistryScannedFile] = useState<File | null>(null);
  const [submittingMinistryDelivery, setSubmittingMinistryDelivery] = useState(false);
  ...
  const fetchTrainees = async (page = 1, limit = 10, resetData = false, customSearchQuery?: string) => {
    try {
      ...
      const normalizedSearch = normalizeSearchText(currentSearchQuery || '');
  ...
  const handleSearch = () => {
    ...
    setSearchQuery(normalizedSearch); // تطبيق البحث
    ...
    fetchTrainees(1, pagination.limit, true, normalizedSearch);
  };
  ...
  const getMinistryDeliveryStatus = async (traineeId: number) => {
    const data = await fetchAPI(`/ministry-exam-declarations/admin/trainee/${traineeId}/delivery-status`);
  ...
  const getMinistryPolicyBlockingMessage = (payload: any): string | null => {
    ...
    // إذا كانت الطلبات مفتوحة ويوجد رسم مرتبط غير مسدد بالكامل، نمنع الطباعة/التسليم من الإدارة أيضاً.
    ...
    if (hasLinkedFee && isRequestsOpen && !isFullyPaid) {
      ...
      const feeAmountValue = Number(
        policy?.fee?.amount ?? policy?.payment?.requiredAmount ?? 0,
      );
      const feeAmount = feeAmountValue > 0 ? ` (${feeAmountValue.toLocaleString('ar-EG')} ج.م)` : '';
  ...
  const handleOpenMinistryDeclarationPrint = async (trainee: Trainee) => {
    try {
      const statusPayload = await getMinistryDeliveryStatus(trainee.id);
      ...
      const data = await fetchAPI(`/ministry-exam-declarations/admin/trainee/${trainee.id}/public-link`);
      ...
      if (!url) {
        toast.error('تعذر إنشاء رابط طباعة الإقرار لهذا المتدرب');
        return;
      }

      window.open(url, '_blank', 'noopener,noreferrer');
    } catch (error: any) {
      console.error('Error generating ministry declaration public link:', error);
      toast.error(error.message || 'تعذر فتح نموذج إقرار وزارة العمل');
    }
  };

  const resetMinistryDeliveryModal = () => {
    setShowMinistryDeliveryModal(false);
    setSelectedTraineeForMinistryDelivery(null);
    setMinistryDeliveryNotes('');
    setMinistryScannedFile(null);
    setSubmittingMinistryDelivery(false);
  };

  const handleOpenMinistryDeliveryModal = async (trainee: Trainee) => {
    try {
      const payload = await getMinistryDeliveryStatus(trainee.id);

      const blockingMessage = getMinistryPolicyBlockingMessage(payload);
      if (blockingMessage) {
        toast.error(blockingMessage);
        return;
      }
    ...
    setSelectedTraineeForMinistryDelivery(trainee);
    setMinistryDeliveryNotes('');
    setMinistryScannedFile(null);
    setShowMinistryDeliveryModal(true);
  };

  const handleMinistryScannedFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isAllowedMinistryDeclarationFile(file)) {
      toast.error('يرجى رفع صورة واضحة للإقرار (JPG/PNG/WEBP وغيرها من صيغ الصور)');
      e.target.value = '';
      return;
    }
    ...
    setMinistryScannedFile(file);
  };

  const handleSubmitMinistryDelivery = async () => {
    if (!selectedTraineeForMinistryDelivery) return;

    if (!ministryScannedFile) {
      toast.error('رفع صورة واضحة للإقرار إلزامي قبل الحفظ');
      return;
    }

    try {
      setSubmittingMinistryDelivery(true);

      const statusPayload = await getMinistryDeliveryStatus(selectedTraineeForMinistryDelivery.id);
      const blockingMessage = getMinistryPolicyBlockingMessage(statusPayload);
      if (blockingMessage) {
        toast.error(blockingMessage);
        return;
      ...
      if (statusPayload?.canDeliver === false) {
        toast.error(statusPayload?.message || 'تم استلام الإقرار مسبقاً');
        return;
      }
      ...
      const uploadResult = await uploadFile(ministryScannedFile, 'ministry-declarations');
      payload = {
        ...payload,
        declarationFileUrl: uploadResult.url,
        declarationFileCloudinaryId: uploadResult.public_id || undefined,
        declarationFileName: uploadResult.originalname || ministryScannedFile.name,
        declarationFileMimeType: uploadResult.mimetype || ministryScannedFile.type,
      };

      await fetchAPI(`/ministry-exam-declarations/admin/trainee/${selectedTraineeForMinistryDelivery.id}/deliver`, {
        method: 'POST',
        body: JSON.stringify(payload),
      });

      toast.success('تم تسجيل تسليم إقرار وزارة العمل واعتماده تلقائياً');
      resetMinistryDeliveryModal();
      await fetchTrainees(pagination.page, pagination.limit, false);
    } catch (error: any) {
      console.error('Error delivering ministry declaration:', error);
      toast.error(error.message || 'تعذر تسجيل تسليم إقرار وزارة العمل');
    } finally {
      setSubmittingMinistryDelivery(false);
    }
  };
  ...
  return (
    <div>
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-2 w-full">
                  ...
                  {canAccessTraineeActions ? (
                    <ActionMenu
                      ...
                      canViewTraineeArchive={canViewTraineeArchive}
                      ...
                      onPrintMinistryDeclaration={handleOpenMinistryDeclarationPrint}
                      onDeliverMinistryDeclaration={handleOpenMinistryDeliveryModal}
                    />
                  ) : (
                    <ButtonSkeleton />
                  )}
      ...
      {showMinistryDeliveryModal && selectedTraineeForMinistryDelivery && (
        <div
          className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => {
            if (!submittingMinistryDelivery) {
              resetMinistryDeliveryModal();
            }
          }}
        >
          ...
          >
            <div className="px-6 py-4 border-b border-slate-200">
              ...
              <p className="text-xs text-slate-500 mt-1">
                المتدرب: {selectedTraineeForMinistryDelivery.nameAr} ({selectedTraineeForMinistryDelivery.nationalId || 'بدون رقم قومي'})
              </p>
            </div>
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="space-y-2">
                ...
                <div className="p-3 rounded-xl border border-indigo-200 bg-indigo-50">
                  ...
                  <p className="text-xs text-slate-600 mt-1">
                    التسليم من صفحة المتدربين يتم دائمًا كتسليم فرع، وسيظهر في جميع الصفحات على أنه "من خلال الفرع".
                  </p>
                </div>
              </div>

              <div className="space-y-3 border border-indigo-200 rounded-xl p-3 bg-indigo-50/60">
                <p className="text-sm font-bold text-indigo-900">رفع صورة الإقرار (إجباري)</p>
                <p className="text-xs text-indigo-800 leading-6">
                  يجب رفع صورة واضحة للإقرار بعد التوقيع عليه قبل حفظ التسليم.
                </p>
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-slate-700">ارفع صورة الإقرار</label>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleMinistryScannedFileChange}
                    disabled={submittingMinistryDelivery}
                  ...
                  />
                  {ministryScannedFile ? (
                    <p className="text-xs text-indigo-700 font-medium">تم اختيار: {ministryScannedFile.name}</p>
                  ) : (
                    <p className="text-xs text-slate-500">الأنواع المسموحة: ملفات الصور فقط (حد أقصى 10MB).</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  onChange={(e) => setMinistryDeliveryNotes(e.target.value)}
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-800">
                بمجرد الحفظ سيتم تسجيل الإقرار كتسليم فرع واعتماده تلقائياً مع حفظ الصورة المرفوعة.
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-slate-200 flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
              <Button
                variant="outline"
                onClick={resetMinistryDeliveryModal}
                disabled={submittingMinistryDelivery}
              >
                إلغاء
              </Button>
              <Button
                onClick={handleSubmitMinistryDelivery}
                disabled={submittingMinistryDelivery || !ministryScannedFile}
                className="bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                {submittingMinistryDelivery ? 'جاري الحفظ...' : 'حفظ التسليم واعتماد الإقرار'}
              </Button>
            </div>
          </div>
        </div>
  ...
  const renderMenuItems = () => (
    <>
      ...
      <div className="px-2 py-1">
        <span className="text-xs text-slate-500 font-medium">إقرار اختبار وزارة العمل</span>
      </div>

      <button
        onClick={async () => {
          setIsOpen(false);
          await onPrintMinistryDeclaration(trainee);
        }}
        className="flex items-center px-4 py-3 md:py-2 text-sm text-slate-700 hover:bg-sky-50 w-full text-right transition-colors duration-150 active:bg-sky-100"
        role="menuitem"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="ml-3 md:ml-2 h-5 w-5 md:h-4 md:w-4 text-sky-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        ...
        <span className="font-medium">طباعة إقرار وزارة العمل</span>
      </button>

      <button
        onClick={async () => {
          setIsOpen(false);
          await onDeliverMinistryDeclaration(trainee);
        }}
        className="flex items-center px-4 py-3 md:py-2 text-sm text-slate-700 hover:bg-emerald-50 w-full text-right transition-colors duration-150 active:bg-emerald-100"
        role="menuitem"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="ml-3 md:ml-2 h-5 w-5 md:h-4 md:w-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <span className="font-medium">تسليم إقرار وزارة العمل</span>
      </button>
