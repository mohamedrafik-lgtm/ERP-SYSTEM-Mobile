...
function CheckInPageContent() {
  ...
  // Timer — counts from max(checkInTime, workStartTime)
  useEffect(() => {
    if (status?.todayLog?.checkInTime && !status.todayLog.checkOutTime) {
      const startTime = new Date(status.todayLog.checkInTime).getTime();
      const workStart = status.settings?.workStartTime || '09:00';
      const [wsH, wsM] = workStart.split(':').map(Number);
      const workStartDate = new Date(); workStartDate.setHours(wsH, wsM, 0, 0);
      const effectiveStart = Math.max(startTime, workStartDate.getTime());
      const tick = () => {
        const now = Date.now();
        if (now < effectiveStart) { setElapsed(0); return; }
  ...
  }, [status?.todayLog?.checkInTime, status?.todayLog?.checkOutTime, status?.settings?.workStartTime]);
        ...
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-tiba-gray-200 p-6 md:p-8 text-center shadow-sm">
            <div className="mb-6">
              ...
              {isCheckedIn && (() => {
                const checkInDate = new Date(status.todayLog!.checkInTime!);
                const workStart = status.settings?.workStartTime || '09:00';
                const workEnd = status.settings?.workEndTime || '17:00';
                const [wsH, wsM] = workStart.split(':').map(Number);
                ...
                const toStartSec = Math.max(0, Math.floor((workStartDate.getTime() - nowMs) / 1000));
                const toStartH = Math.floor(toStartSec / 3600);
                const toStartM = Math.floor((toStartSec % 3600) / 60);

                return (
                  <div className="mb-4 animate-[fadeIn_0.3s_ease-out]">
                    {isEarlyCheckIn && isBeforeWorkStart ? (
                      <>
                        <p className="text-xs text-amber-500 font-semibold mb-1">تم التسجيل مبكراً</p>
                        <p className="text-lg font-bold text-amber-600">الدوام يبدأ بعد {toStartH > 0 ? `${toStartH} ساعة و ` : ''}{toStartM} دقيقة</p>
                        <p className="text-[10px] text-tiba-gray-400 mt-1">سيبدأ العد عند الساعة {workStart}</p>
                      </>
                    ) : (
                      <>
                        <p className="text-xs text-tiba-gray-400 mb-1">مدة العمل الفعلية</p>
                        <p className="text-4xl font-bold text-tiba-primary-600 tabular-nums tracking-wider" dir="ltr">
                          {formatDuration(elapsed)}
                        </p>
                        {remainingToEndSec > 0 && (
                          <p className="text-xs text-tiba-primary-500 font-medium mt-1">متبقي على نهاية الدوام: {remainingH > 0 ? `${remainingH} ساعة و ` : ''}{remainingM} دقيقة</p>
                        )}
                      </>
                    )}
                  </div>
