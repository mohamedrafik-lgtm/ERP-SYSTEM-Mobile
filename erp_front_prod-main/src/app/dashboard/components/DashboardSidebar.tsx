...
import {
  ...
  FiAlertCircle, FiDownload, FiInbox, FiBell, FiUpload, FiCopy, FiAlertTriangle, FiTrash2, FiSearch
} from 'react-icons/fi';
import { 
...
export default function DashboardSidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [pendingRequestCounts, setPendingRequestCounts] = useState<{
    deferral: number; general: number; complaints: number; gradeAppeals: number; ministryDeclaration: number;
  }>({ deferral: 0, general: 0, complaints: 0, gradeAppeals: 0, ministryDeclaration: 0 });
  const [liveCrmAccess, setLiveCrmAccess] = useState(user?.hasCrmAccess || false);
  const [quickAccessOpen, setQuickAccessOpen] = useState(false);
  const [quickQuery, setQuickQuery] = useState('');

  // جلب صلاحية CRM الحية من الـ API
  useEffect(() => {
    if (user?.id) {
      fetchAPI('/auth/profile').then((profile: any) => {
        setLiveCrmAccess(profile?.hasCrmAccess || false);
      }).catch(() => {});
    }
  }, [user?.id]);
  ...
  useEffect(() => {
    const fetchPendingCounts = async () => {
      const counts = { deferral: 0, general: 0, complaints: 0, gradeAppeals: 0, ministryDeclaration: 0 };
      ...
      if (hasPermission('dashboard.ministry-exam-declarations', 'view')) {
        fetches.push(
          fetchAPI('/ministry-exam-declarations/stats').then(d => {
            counts.ministryDeclaration = (d?.pendingReview || 0) + (d?.pendingBranchDelivery || 0);
          }).catch(() => {})
        );
      }
  ...
  const fuzzyScore = (query: string, target: string) => {
    ...
    if (!q || !t) return 0;
    if (t.includes(q)) return 100 - Math.min(30, t.indexOf(q));
    ...
    qWords.forEach((qw) => {
      ...
      tWords.forEach((tw) => {
        ...
        if (ratio >= 0.55) {
          wordBest = Math.max(wordBest, Math.round(ratio * 70));
        }
      });
      score += wordBest;
    });
  ...
  const sidebarCategories: SidebarCategory[] = [

    {
    ...
    },
      {
        ...
        requiredPermissions: [
          ...
          { resource: 'dashboard.schedule', action: 'view' },
    ...
    {
      title: 'ادارة التوزيعات و التحويلات',
    ...
    {
      title: 'إدارة الكنترول و الدرجات',
      icon: <FiClipboard className="w-5 h-5" />,
      requiredPermissions: [
        ...
        { resource: 'dashboard.grades', action: 'view' },
    ...
    },
    {
      ...
      requiredPermissions: [
        ...
        { resource: 'dashboard.ministry-exam-fees', action: 'view' },
      ],
      requireAll: false,
      items: [
        {
        ...
        {
          title: 'رسوم اختبار وزارة العمل',
          href: '/dashboard/requests/ministry-exam-fees',
          icon: <FiAward className="w-5 h-5" />,
          requiredPermission: { resource: 'dashboard.ministry-exam-fees', action: 'view' },
          subGroup: 'الرسوم المالية',
        },
        {
    ...
    },
    {
      ...
      requiredPermissions: [
        ...
        { resource: 'staff-attendance.overtime', action: 'view' },
      ],
      requireAll: false,
      items: [
        {
        ...
        {
          title: 'الأوقات الإضافية',
          href: '/dashboard/staff-attendance/overtime',
          icon: <FiClock className="w-5 h-5" />,
          requiredPermission: { resource: 'staff-attendance.overtime', action: 'view' },
          subGroup: 'نظام الحضور والغياب',
        },
        {
    ...
    },
    {
      ...
      icon: <ComputerDesktopIcon className="w-5 h-5" />,
      badge: (pendingRequestCounts.deferral + pendingRequestCounts.general + pendingRequestCounts.complaints + pendingRequestCounts.gradeAppeals + pendingRequestCounts.ministryDeclaration) || undefined,
      requiredPermissions: [
        { resource: 'dashboard.trainee-platform', action: 'view' },
        { resource: 'dashboard.trainee-platform.accounts', action: 'view' },
        { resource: 'dashboard.trainee-platform.stats', action: 'view' },
        { resource: 'dashboard.surveys', action: 'view' },
        ...
        { resource: 'dashboard.ministry-exam-declarations', action: 'view' },
        { resource: 'dashboard.ministry-exam-declarations-review', action: 'view' },
      ],
      requireAll: false,
      items: [
        {
        ...
        {
          title: 'إقرار اختبار وزارة العمل',
          href: '/dashboard/requests/ministry-exam-declarations-review',
          icon: <FiClipboard className="w-5 h-5" />,
          badge: pendingRequestCounts.ministryDeclaration || undefined,
          requiredPermission: { resource: 'dashboard.ministry-exam-declarations-review', action: 'view' },
        },
        {
    ...
    },
    {
      ...
      requiredPermissions: [
        ...
        { resource: 'dashboard.permissions', action: 'manage' },
        { resource: 'dashboard.settings', action: 'view' },
        { resource: 'dashboard.settings', action: 'edit' },
        { resource: 'dashboard.settings.locations', action: 'manage' },
        { resource: 'dashboard.developer-settings', action: 'view' },
        { resource: 'dashboard.backup', action: 'view' },
      ],
      requireAll: false,
      items: [
        {
    ...
    },
    {
      ...
      items: [
        ...
        {
          title: 'أوقاتي الإضافية',
          href: '/dashboard/staff-attendance/my-overtime',
          icon: <FiClock className="w-5 h-5" />,
        },
      ]
    },
  ...
  const visibleSidebarCategories = useMemo(
    () => sidebarCategories.filter(canShowCategory),
    [sidebarCategories, permissionsLoading, pendingRequestCounts, liveCrmAccess, isStaffEnrolled]
  );

  const quickAccessEntries = useMemo(() => {
    const entriesFromSidebar = visibleSidebarCategories.flatMap((category) => {
    ...
    const topEntries = [
      { title: 'الرئيسية', href: '/dashboard', categoryTitle: 'عام' },
      { title: 'الملف الشخصي', href: '/dashboard/profile', categoryTitle: 'عام' },
      { title: 'المحادثات', href: '/dashboard/chat', categoryTitle: 'عام' },
      { title: 'الإشعارات', href: '/dashboard/notifications', categoryTitle: 'عام' },
      ...(liveCrmAccess || hasPermission('crm', 'access')
        ? [{ title: 'نظام CRM', href: '/crm', categoryTitle: 'الأنظمة' }]
        : []),
    ];

    const merged = [...entriesFromSidebar, ...topEntries];

    // إزالة التكرارات بنفس الرابط
    const uniqueByHref = Array.from(new Map(merged.map((e) => [e.href, e])).values());
    return uniqueByHref;
  }, [visibleSidebarCategories, permissionsLoading, pendingRequestCounts, liveCrmAccess, isStaffEnrolled, pathname]);

  const quickAccessResults = useMemo(() => {
    const q = quickQuery.trim();
    if (!q) return quickAccessEntries.slice(0, 12);

    return quickAccessEntries
      .map((entry) => {
        const haystack = `${entry.title} ${entry.categoryTitle} ${entry.subGroup || ''}`;
      ...
      .sort((a, b) => b.score - a.score)
        .slice(0, 18);
  }, [quickQuery, quickAccessEntries]);

  const isQuickQueryEmpty = quickQuery.trim().length === 0;
  ...
  const handleQuickAccessNavigate = (href: string) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="space-y-1 mt-2">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all duration-300 group"
            >
              <div className="flex items-center gap-3">
                <FiSearch className="w-5 h-5 group-hover:scale-110 transition-transform" />
                <span className="font-bold text-sm">الوصول السريع</span>
              </div>
              <span className="text-[11px] px-2 py-1 rounded-md bg-white/15">Ctrl K</span>
            </button>
        ...
        <div className="p-4 bg-blue-800/50 space-y-2">
          ...
          {(liveCrmAccess || hasPermission('crm', 'access')) && (
            <Link
              href="/crm"
              className="flex items-center justify-center gap-3 w-full px-4 py-3 text-sm text-white bg-emerald-500/20 hover:bg-emerald-500/30 rounded-xl transition-all duration-300 font-bold group"
            >
              <FiUsers className="w-5 h-5 transition-transform duration-300 group-hover:scale-110" />
              <span>نظام CRM</span>
            </Link>
          )}
          {/* زر تسجيل الخروج */}
      ...
      {quickAccessOpen && (
        <div
          className="fixed inset-0 z-[90] bg-slate-950/70 backdrop-blur-sm p-0 sm:p-6 flex items-stretch sm:items-center justify-center"
          onClick={() => {
            setQuickQuery('');
            setQuickAccessOpen(false);
          }}
        >
          <div
            className="w-full h-full sm:max-w-4xl sm:h-[72vh] rounded-none sm:rounded-2xl border-0 sm:border sm:border-slate-300/25 bg-gradient-to-b from-slate-100 to-white shadow-none sm:shadow-[0_24px_60px_rgba(2,6,23,0.45)] flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="border-b border-slate-300/70 p-3 sm:p-4 bg-white/85 backdrop-blur">
              <div className="flex items-center justify-between gap-3 mb-2">
                <div className="text-right">
                  <p className="text-slate-900 font-extrabold text-base sm:text-lg">الوصول السريع</p>
                  <p className="text-slate-600 text-xs sm:text-sm">ابحث باسم الصفحة أو القسم مع دعم الأخطاء الإملائية</p>
                </div>
                <button
                  onClick={() => {
                    setQuickQuery('');
                  ...
                  className="p-2 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-200/70"
                >
                  <FiX className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="relative">
                <FiSearch className="w-4 h-4 text-slate-500 absolute right-3 top-1/2 -translate-y-1/2" />
                <input
                  autoFocus
                  value={quickQuery}
                  onChange={(e) => setQuickQuery(e.target.value)}
                  placeholder="اكتب اسم الصفحة أو المهمة..."
                  className="w-full rounded-xl border border-slate-300 bg-white px-10 py-2.5 text-slate-900 placeholder:text-slate-500 outline-none ring-0 focus:border-blue-600 focus:ring-2 focus:ring-blue-100 text-sm sm:text-base"
                />
              </div>
            </div>

            <div className="flex-1 overflow-y-auto p-2 sm:p-3 bg-slate-100/70">
              {isQuickQueryEmpty && quickAccessResults.length > 0 && (
                <div className="px-1 pb-2 text-right">
                  <p className="text-xs sm:text-sm text-slate-600 font-bold">اقتراحات سريعة</p>
                </div>
              )}
              {quickAccessResults.length === 0 ? (
                <div className="h-full flex items-center justify-center text-slate-600 text-sm sm:text-base">
                  لا توجد نتائج مطابقة. جرّب كتابة كلمة قريبة من اسم الصفحة.
                </div>
              ) : (
                <div className="space-y-1.5">
                  {quickAccessResults.map((entry) => (
                    <button
                      key={`${entry.href}-${entry.title}`}
                      onClick={() => handleQuickAccessNavigate(entry.href)}
                      className="w-full text-right rounded-lg border border-slate-200 bg-white hover:bg-blue-50 hover:border-blue-300 transition-all px-3 py-2.5"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <p className="text-slate-900 font-bold text-sm sm:text-[15px] truncate">{entry.title}</p>
                          <p className="text-slate-600 text-xs sm:text-sm truncate">
                            {entry.subGroup ? `${entry.categoryTitle} • ${entry.subGroup}` : entry.categoryTitle}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          <span className="hidden sm:inline-flex text-[11px] px-2 py-1 rounded-md bg-slate-100 text-slate-700 font-bold">
                            {entry.categoryTitle}
                          </span>
                          <span className="text-blue-700 text-xs sm:text-sm font-extrabold">فتح</span>
                        </div>
                      </div>
                    </button>
                  ))}
