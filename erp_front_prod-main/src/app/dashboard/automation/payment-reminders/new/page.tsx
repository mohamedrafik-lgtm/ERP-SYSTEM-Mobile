...
function NewReminderPageContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <PageHeader
        title="إنشاء رسالة تذكير جديدة"
        description="إعداد رسالة تذكير تلقائية للمتدربين"
        breadcrumbs={[
      ...
      />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {fees.length === 0 && (
                    <div className="mt-3 bg-tiba-warning-50 border border-tiba-warning-200 rounded-lg p-4">
                      <p className="text-sm text-tiba-warning-800 mb-2 flex items-center gap-2">
                  ...
                  {fees.length > 0 && (
                    <p className="text-xs text-tiba-gray-500 mt-2 flex items-center gap-1">
                      <CheckIcon className="text-green-600 h-3 w-3" />
                ...
                {Object.entries(TRIGGER_TYPE_LABELS).map(([value, label]) => (
                  <label
                    ...
                    className={`flex items-start gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                      formData.triggerType === value ? 'border-tiba-primary-500 bg-tiba-primary-50 shadow-md' : 'border-tiba-gray-200 hover:border-tiba-primary-300'
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    <div className="flex-1">
                      <p className="text-base font-bold text-tiba-gray-900 mb-1">{label}</p>
                      <p className="text-sm text-tiba-gray-600">{TRIGGER_TYPE_DESCRIPTIONS[value as ReminderTriggerType]}</p>
                    </div>
                    {formData.triggerType === value && (
                      <CheckIcon className="w-5 h-5 text-tiba-primary-600" />
                    )}
                  </label>
                ))}
              </div>
          ...
          {/* Important Notice */}
          <Card className="border-2 border-tiba-primary-300 bg-tiba-primary-50">
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                <div className="bg-tiba-primary-600 p-3 rounded-full flex-shrink-0">
                  <InformationCircleIcon className="h-6 w-6 text-white" />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-tiba-primary-900 mb-2">ℹ️ ملاحظة هامة</h3>
                  <div className="space-y-2 text-sm text-tiba-primary-800">
                    <p className="flex items-center gap-2">
                      <UsersIcon className="h-4 w-4 text-tiba-primary-600" />
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              ...
              <Button
                type="submit"
                disabled={saving}
                className="bg-tiba-primary-600 hover:bg-tiba-primary-700 text-white font-bold px-8 py-3 shadow-sm disabled:opacity-50"
              >
          ...
          <AlertDialogContent className="!bg-white border-2 border-blue-200 shadow-2xl max-w-md">
            <AlertDialogHeader>
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-tiba-primary-100 p-3 rounded-full">
                  <BellIcon className="h-6 w-6 text-tiba-primary-600" />
                </div>
            ...
            <AlertDialogFooter className="gap-3 mt-4">
              ...
              <AlertDialogAction
                ...
                disabled={saving}
                className="bg-tiba-primary-600 hover:bg-tiba-primary-700 text-white font-bold px-6 py-2.5 shadow-sm disabled:opacity-50"
              >
                {saving ? (
                  <>
                  ...
                  <>
                    <CheckIcon className="w-4 h-4 ml-2 inline-block" />
                    تأكيد الإنشاء
                  </>
                )}
              </AlertDialogAction>
