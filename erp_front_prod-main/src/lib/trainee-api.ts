...
export const traineeAPI = {
  ...
  // إرسال نبضة heartbeat للتتبع
  heartbeat: async (page?: string) => {
    const sessionToken = typeof window !== 'undefined' ? localStorage.getItem('trainee_session_token') : null;
    if (!sessionToken) return null;
    return fetchTraineeAPI('trainee-auth/heartbeat', {
      method: 'POST',
