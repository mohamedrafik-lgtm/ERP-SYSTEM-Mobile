  ...
  if (pathname.startsWith('/crm')) {
    ...
    try {
      ...
      try {
        ...
        // المحاضرون لا يمكنهم الوصول لنظام CRM
        if (tokenData.accountType === 'INSTRUCTOR') {
          return NextResponse.redirect(new URL('/instructor-dashboard', request.url));
        }

        // التحقق من صلاحية CRM: يجب أن يكون لديه hasCrmAccess أو نوع حساب/دور إداري
        const isAdminAccount = ['SUPER_ADMIN', 'SYSTEM_ADMIN', 'ADMIN'].includes(tokenData.accountType);
        const isAdminRole = ['super_admin', 'system_admin', 'admin'].includes(tokenData.role);
        if (!tokenData.hasCrmAccess && !isAdminAccount && !isAdminRole) {
          console.log('Middleware - User without CRM access, redirecting to /dashboard');
          return NextResponse.redirect(new URL('/dashboard', request.url));
        }

        console.log('Middleware - Valid CRM access, allowing entry');
      } catch (parseError) {
        const response = NextResponse.redirect(new URL('/login', request.url));
        response.cookies.delete('auth_token');
        return response;
...
export const config = {
  matcher: ["/dashboard/:path*", "/instructor-dashboard/:path*", "/crm/:path*", "/login", "/instructor-login", "/trainee-auth", "/trainee-dashboard/:path*"],
};
