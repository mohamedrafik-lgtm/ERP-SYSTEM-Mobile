...
export const RolePermissionsModal: React.FC<RolePermissionsModalProps> = ({
  ...
  if (isSuperAdmin) {
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden border border-tiba-gray-200">
          ...
          <div className="p-6 text-center">
            ...
            <div className="bg-tiba-primary-50 border border-tiba-primary-200 rounded-lg p-3 mb-4">
              <div className="flex items-center gap-2 text-tiba-primary-700">
                <InformationCircleIcon className="h-4 w-4" />
                <span className="text-sm font-medium">معلومة</span>
              </div>
              <p className="text-sm text-tiba-primary-600 mt-1">
                يتم منح جميع الصلاحيات الجديدة لهذا الدور تلقائياً عند إنشائها
              </p>
            </div>
            <Button variant="secondary" onClick={resetModal} fullWidth>
              فهمت
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-tiba-gray-200">
        {/* Header */}
        ...
        {/* Filters */}
        <div className="px-6 py-4 border-b border-tiba-gray-200 bg-tiba-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-sm text-tiba-gray-700">
                <input
                  type="checkbox"
                  checked={showDependencies}
                  onChange={(e) => setShowDependencies(e.target.checked)}
                  className="rounded border-tiba-gray-300 text-tiba-primary-600 focus:ring-tiba-primary-500"
                />
                <LinkIcon className="h-3 w-3 text-tiba-primary-500" />
                تطبيق الترابط التلقائي للصلاحيات
              </label>
            </div>

            {!validationResult.isValid && (
              <div className="flex items-center gap-2 text-tiba-warning-600">
          ...
          {/* رسائل التحذير */}
          {!validationResult.isValid && (
            <div className="mt-3 p-3 bg-tiba-warning-50 border border-tiba-warning-200 rounded-lg">
              <div className="flex items-start gap-2">
                <ExclamationTriangleIcon className="h-4 w-4 text-tiba-warning-500 mt-0.5 flex-shrink-0" />
                <div className="flex-1">
                  <h4 className="text-sm font-medium text-tiba-warning-800 mb-1">
                    صلاحيات مطلوبة مفقودة
                  </h4>
                  <ul className="text-xs text-tiba-warning-700 space-y-1">
                    {validationResult.suggestions.slice(0, 3).map((suggestion, index) => (
                      <li key={index}>• {suggestion}</li>
                    ))}
                    {validationResult.suggestions.length > 3 && (
                      <li className="text-tiba-warning-600">
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {loadingPermissions ? (
            <div className="text-center py-8">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-tiba-primary-600 border-t-transparent mx-auto mb-4"></div>
              <p className="text-tiba-gray-600">جاري تحميل الصلاحيات...</p>
            </div>
          ) : (
            <div className="space-y-6">
              ...
              {Object.entries(groupedPermissions).map(([category, permissions]) => (
                <div key={category} className="bg-tiba-gray-50 rounded-lg p-4 border border-tiba-gray-200">
                  <h3 className="font-semibold text-tiba-gray-800 mb-3 flex items-center gap-2">
                    <KeyIcon className="h-4 w-4 text-tiba-primary-500" />
                    ...
                    {permissions.map((permission) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isImplied && (
                            <div className="absolute -top-1 -right-1 w-4 h-4 bg-tiba-primary-500 rounded-full flex items-center justify-center">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-medium text-sm text-tiba-gray-800">
                                  {permission.displayName}
                                </span>
                                {isGranted && (
                                  <CheckIcon className={`h-3 w-3 ${isImplied ? 'text-tiba-primary-600' : 'text-green-600'}`} />
                                )}
                                {isImplied && (
                                  <span className="text-xs bg-tiba-primary-100 text-tiba-primary-700 px-1.5 py-0.5 rounded">
                                    مُضمنة
                                  </span>
                                )}
                              </div>
                              <p className="text-xs text-tiba-gray-600 mb-1">
                                {permission.description}
                              </p>
                              <div className="text-xs font-mono text-tiba-gray-500 mb-2">
                              ...
                              {(relatedPerms.required.length > 0 || relatedPerms.implied.length > 0) && (
                                <div className="text-xs space-y-1">
                                  {relatedPerms.required.length > 0 && (
                                    <div className="flex items-center gap-1 text-tiba-warning-600">
                                  ...
                                  {relatedPerms.implied.length > 0 && (
                                    <div className="flex items-center gap-1 text-tiba-primary-600">
                            ...
                            <div className="ml-3">
                              <div
                                className={`
                                  ...
                                  ${isGranted
                                    ? isImplied
                                      ? 'bg-tiba-primary-500 border-tiba-primary-500'
                                      : 'bg-green-500 border-green-500'
                                    : 'border-tiba-gray-300'
                                  }
                                `}
                              >
                                {isGranted && (
        ...
        {/* Footer */}
        <div className="flex-shrink-0 px-6 py-4 border-t border-tiba-gray-200 bg-tiba-gray-50 sticky bottom-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-4 text-sm text-tiba-gray-600">
              <span>{grantedCount} من {allPermissions.length} صلاحية ممنوحة</span>
              {showDependencies && (
                <span className="flex items-center gap-1 text-tiba-primary-600">
                  <LinkIcon className="h-3 w-3" />
                  الترابط مُفعل
                </span>
              )}
              {!validationResult.isValid && (
                <span className="flex items-center gap-1 text-tiba-warning-600">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-shrink-0 w-full sm:w-auto">
              <Button variant="outline" onClick={resetModal} className="order-2 sm:order-1">
                إلغاء
              </Button>
              <Button
                onClick={handleSave}
                isLoading={loading}
                variant={!validationResult.isValid ? 'warning' : 'default'}
                leftIcon={!validationResult.isValid ? <ExclamationTriangleIcon className="h-4 w-4" /> : <KeyIcon className="h-4 w-4" />}
                className="order-1 sm:order-2"
              >
                {!validationResult.isValid ? 'حفظ مع التحذيرات' : 'حفظ التغييرات'}
              </Button>
            </div>
          </div>

          {/* شرح الرموز */}
          <div className="flex items-center gap-6 text-xs text-tiba-gray-500 pt-2 border-t border-tiba-gray-200">
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-green-500 rounded"></div>
              <span>صلاحية مُختارة</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-tiba-primary-500 rounded relative">
                <LinkIcon className="h-1.5 w-1.5 text-white absolute top-0.5 right-0.5" />
              </div>
              <span>صلاحية مُضمنة تلقائياً</span>
            </div>
            <div className="flex items-center gap-1">
              <ExclamationTriangleIcon className="h-3 w-3 text-tiba-warning-500" />
              <span>صلاحية مطلوبة</span>
            </div>
            <div className="flex items-center gap-1">
              <InformationCircleIcon className="h-3 w-3 text-tiba-primary-500" />
