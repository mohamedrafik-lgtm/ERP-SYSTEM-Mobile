...
import type { AccountType, UpdateUserRequest, UserItem } from '../types/users';
...
const EditUserScreen = ({ navigation, route }: any) => {
  const routeUserId = route?.params?.userId || route?.params?.user?.id;
  ...
  const activeProgramIds = useMemo(() => {
    if (programAccessMode === 'all') return programs.map((p) => p.id);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      if (!routeUserId) {
        Toast.show({ type: 'error', text1: 'بيانات غير مكتملة', text2: 'لم يتم تحديد المستخدم' });
        navigation.goBack();
        return;
      }
      ...
      try {
        ...
        const [rolesData, programsData, userData] = await Promise.all([
          AuthService.getRoles(),
          AuthService.getAllPrograms(),
          AuthService.getUserById(routeUserId),
        ]);
        ...
        setHasCrmAccess(!!userData.hasCrmAccess);
        setIsArchived(!!userData.isArchived);

        const roleIds = (userData.userRoles || []).filter((r) => r.isActive).map((r) => r.roleId);
        setSelectedRoleIds(roleIds);

        const allowedProgramIds = userData.allowedProgramIds || [];
        setSelectedProgramIds(allowedProgramIds);
        setProgramAccessMode(allowedProgramIds.length > 0 ? 'specific' : 'all');

        const allowedDistributionIds = userData.allowedDistributionIds || [];
        setSelectedDistributionIds(allowedDistributionIds);
        setDistributionAccessMode(allowedDistributionIds.length > 0 ? 'specific' : 'all');

        const allowedRoomIds = userData.allowedRoomIds || [];
        setSelectedRoomIds(allowedRoomIds);

        const modeMap: Record<string, 'all' | 'specific'> = {};
        const allowedRoomsList = userData.allowedRoomsList || [];
        for (const distributionId of allowedDistributionIds) {
          const hasSpecificRooms = allowedRoomsList.some((room) => room.distributionId === distributionId);
          modeMap[distributionId] = hasSpecificRooms ? 'specific' : 'all';
        }
        setDistributionRoomMode(modeMap);
      } catch (e: any) {
        Toast.show({ type: 'error', text1: 'فشل تحميل بيانات المستخدم', text2: e?.message || 'حدث خطأ' });
        navigation.goBack();
      } finally {
  ...
  useEffect(() => {
    const loadDistributions = async () => {
      ...
      try {
        ...
        const responses = await Promise.all(activeProgramIds.map((programId) => AuthService.getTraineeDistributions({ programId })));
  ...
  const toggleProgram = (programId: number) => {
    setSelectedProgramIds((prev) => {
      ...
      if (!next.includes(programId)) {
        setSelectedDistributionIds((current) =>
          current.filter((distributionId) => {
  ...
  const validate = (): string | null => {
    ...
    if (form.phone && form.phone.trim() && !/^[0-9+\-\s()]{10,15}$/.test(form.phone.trim())) return 'رقم الهاتف غير صالح';
    if (form.password && form.password.length < 6) return 'كلمة المرور يجب أن تكون 6 أحرف على الأقل';
    if (form.password && form.password !== form.confirmPassword) return 'كلمة المرور غير متطابقة';
  ...
  const onSubmit = async () => {
    if (!routeUserId || !currentUser) return;
    ...
    try {
      setSubmitting(true);
      await AuthService.updateUser(routeUserId, payload);

      const currentRoleIds = (currentUser.userRoles || []).filter((r) => r.isActive).map((r) => r.roleId);
      ...
      const [removeResults, addResults] = await Promise.all([
        Promise.allSettled(rolesToRemove.map((roleId) => AuthService.removeUserRole(routeUserId, roleId))),
        Promise.allSettled(rolesToAdd.map((roleId) => AuthService.assignUserRole(routeUserId, roleId))),
      ]);
      ...
      if (failedRemoves || failedAdds) {
        Toast.show({ type: 'error', text1: 'تم تحديث المستخدم', text2: `تعذر تعديل بعض الأدوار (${failedAdds + failedRemoves})` });
      } else {
        Toast.show({ type: 'success', text1: 'تم تحديث المستخدم بنجاح' });
      }
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          ...
          <View style={styles.switchRow}>
            ...
            <Switch value={hasCrmAccess} onValueChange={setHasCrmAccess} />
          </View>

          <View style={styles.switchRow}>
            <Text style={styles.label}>الحساب مؤرشف</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.segmentRow}>
            <TouchableOpacity style={[styles.segmentBtn, programAccessMode === 'all' && styles.segmentBtnActive]} onPress={() => { setProgramAccessMode('all'); setSelectedProgramIds([]); }}>
        ...
        <View style={styles.card}>
          <Text style={styles.title}>كلمة المرور (اختياري)</Text>
          <View style={styles.inputGroup}>
            <Text style={styles.label}>كلمة المرور الجديدة</Text>
            <TextInput style={styles.input} secureTextEntry value={form.password} onChangeText={(t) => setField('password', t)} placeholder="اتركها فارغة للاحتفاظ بالحالية" />
          </View>
          <View style={styles.inputGroup}>
        ...
        <TouchableOpacity style={[styles.submitBtn, submitting && styles.submitDisabled]} onPress={onSubmit} disabled={submitting}>
          {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.submitText}>حفظ التعديلات</Text>}
        </TouchableOpacity>
